...
function Block({ block }: { block: ArticleBlock }) {
  switch (block.type) {
    ...
    case "p":
      return (
        <p className="article-block max-w-[720px] mx-auto mt-4 text-[17px] md:text-[18px] leading-[1.7] text-black font-medium">
          {block.text}
        </p>
      );
    case "quote":
    ...
    case "list":
      return block.ordered ? (
        <ol className="article-block max-w-[720px] mx-auto mt-5 list-decimal pl-5 space-y-2 text-[17px] md:text-[18px] leading-[1.7] text-black font-medium">
          {block.items.map((item) => (
            <li key={item}>{item}</li>
          ))}
        </ol>
      ) : (
        <ul className="article-block max-w-[720px] mx-auto mt-5 list-disc pl-5 space-y-2 text-[17px] md:text-[18px] leading-[1.7] text-black font-medium">
          {block.items.map((item) => (
            <li key={item}>{item}</li>
          ))}
        </ul>
